...
**决策：接受整套 affiliate 系统，但默认关闭，由管理员在”系统设置 → 通用设置”中控制开关。**

理由：

- 代码层面：上游已内置 `affiliate_enabled` 功能开关（`SettingKeyAffiliateEnabled`），关闭时注册忽略 `aff_code`、不产生新返利、菜单隐藏，已有返利配额仍可转余额。这正好满足”接受但暂不开通”的需求。
- 管理层面：管理员随时可在后台一键开启，无需二次开发。
- 风险层面：关闭状态下对现有业务零侵入，等未来需要增长分销时再启用即可。

上游实现要点（供移植参考）：

| 要点 | 说明 |
|------|------|
| 功能开关 | `affiliate_enabled` 设置项，全局控制启用/禁用 |
| 全局返利率 | `affiliate_rebate_rate`，管理员在通用设置中配置百分比 |
| 冻结期 | `affiliate_rebate_freeze_hours`，返利额度先冻结、到期自动解冻 |
| 有效期 | `affiliate_rebate_duration_days`，绑定后 N 天内有效 |
| 单人上限 | `affiliate_rebate_per_invitee_cap`，每个被邀请人的返利上限 |
| 用户级自定义 | 支持自定义邀请码和自定义返利率（管理员后台调整） |
| OAuth 集成 | 所有 OAuth 路径（LinuxDo、OIDC、微信、Pending Flow）均支持绑定邀请码 |
| 安全加固 | 邀请码格式校验防时序攻击、同步缓存失效、事务传播保证 |

建议：**P1 吸收。按”数据模型与迁移 → 后端核心 → 管理员开关 → 支付返利勾连 → 前端页面 → OAuth 集成”六步移植，默认关闭。**
...
- affiliate 邀请返利系统及其前后台页面、迁移、支付返利结算联动（**已决定吸收，默认关闭，管理员可控**）。
...
1. **Affiliate 邀请返利系统（已决定吸收，默认关闭）**
   - 目标：移植上游完整 affiliate 系统，通过管理员开关控制启用/禁用。
   - 默认状态：`affiliate_enabled = false`，所有返利功能静默关闭。
   - 实施步骤：
     1. **数据模型与迁移**：移植 migration 130-133（`user_affiliates` 表、约束、冻结期支持）。
     2. **后端核心**：移植 `affiliate_repo.go`、`affiliate_service.go` 及单元/集成测试。
     3. **管理员开关**：在”系统设置 → 通用设置”中新增 affiliate 开关和全局参数（返利率、冻结期、有效期、单人上限），默认关闭。
     4. **支付返利勾连**：在 `payment_fulfillment.go` 中接入返利逻辑，仅在开关开启时生效。
     5. **前端页面**：移植用户 affiliate 页面、管理员设置区块、注册页邀请码输入框；关闭时菜单隐藏。
     6. **OAuth 集成**：所有 OAuth 路径支持 `aff_code` 参数绑定。
   - 关键约束：每步移植后确保测试通过，关闭状态下对现有功能零侵入。

2. **EasyPay/ZPay 退款端点专项审计**
   - 目标：核对退款请求路径、参数签名、返回解析与错误处理。
   - 重点：真实 provider 差异、退款失败重试边界、单测覆盖。

3. **Anthropic usage 语义一致性审计**
...
### 第三阶段：移植 affiliate 系统（默认关闭）

- 按上述 P1 六步移植 affiliate 系统，默认 `affiliate_enabled = false`。
- 每步移植后运行全量测试，确保关闭状态下零侵入。
- 管理员可随时在”系统设置 → 通用设置”中开启。

## 最终判断

- **有启发，而且有必要吸收一部分。**
- 最值得吸收的不是”完整跟进上游 47 个 commit”，而是：
  - `P0` Claude Code mimicry/prompt cache 正确性；
  - `P0` Responses pre-output failover；
  - `P1` Affiliate 邀请返利系统（**已决定吸收，默认关闭，管理员可一键开启**）；
  - `P1` EasyPay/ZPay 退款端点；
  - `P1` Anthropic usage 语义一致性。
- **affiliate 邀请返利系统**：上游已内置 `affiliate_enabled` 功能开关，关闭时注册忽略邀请码、不产生返利、菜单隐藏，对现有业务零侵入。管理员随时可在”系统设置 → 通用设置”中开启，无需二次开发。
