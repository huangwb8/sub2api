...
Sub2API 内置支付系统，支持用户自助充值、购买订阅、续费订阅以及在同一升级族内按剩余价值补差价升级，无需部署独立的支付服务。

当前前端展示口径约定如下：充值、支付、订单金额与用户余额统一按人民币（CNY / `¥`）展示；API 使用成本、额度限制以及订阅分组的 `*_usd` 限额继续按美元（USD / `$`）展示，二者不做自动汇率换算。
...
如果还要开放普通用户自助订阅或补差价升级，请继续在“订阅管理”中维护可售套餐，并为可升级套餐配置 `upgrade_family` 与 `upgrade_rank`。
...
| **每日限额** | 每用户每日累计充值上限，仅作用于余额充值订单（留空表示不限制） | - |
...
> **敏感凭据为只写字段**：服务商的私钥、密钥、Webhook Secret 等敏感字段不会再通过管理端接口回传到浏览器。编辑已有服务商时，这些字段留空表示“保持原值”，只有需要轮换凭据时才重新填写。

> **启用中的服务商会立即校验配置**：保存已启用的微信官方服务商时，系统会立刻校验必填字段、`APIv3 Key` 长度以及 `privateKey/publicKey` 的 PEM 格式，避免拖到用户首单时才暴露配置错误；禁用中的实例仍允许作为半成品草稿保存。
...
直接对接支付宝开放平台，支持 PC 页面支付和 H5 手机网站支付。桌面端会直接打开支付宝收银台页面，不再在站内伪造二维码。
...
> 如果后台提示“缺少必填项”或“密钥格式不正确”，优先检查 `微信支付公钥 ID`、`商户证书序列号` 是否填写完整，以及私钥/公钥是否为合法 PEM 内容。
...
> 负载均衡时，系统会自动跳过超出限额的实例；如果所有候选实例都超限，则直接拒绝创建订单，不会再回退到超限实例继续下单。
...
用户选择充值金额和支付方式
  ...
  支付回调验签 → 订单 PAID
       ...
       ▼
  自动充值到用户余额
  ├─ 若邀请返利已启用且用户存在有效邀请人，累计返利
  └─ 订单 COMPLETED
```

> 邀请返利默认关闭。管理员可在 **设置 → 安全与认证 → 邀请返利** 中开启，并配置全局返利比例、冻结期、有效期与单个被邀请人的返利上限；关闭时现有充值流程不产生返利。
...
- 普通用户可在购买页直接购买订阅套餐，也可以对已有活跃订阅进行续费
- 如果源订阅带有当前套餐快照，并且目标套餐与源套餐处于同一 `upgrade_family` 且 `upgrade_rank` 更高，用户可以在订阅列表中点击“升级套餐”进入补差价流程
- 升级时系统会按源订阅当前计费周期的剩余比例折抵旧套餐价值，优先推荐余额支付；余额不足时可继续走现有扫码或 Stripe 支付链路
- 如果差价为 0，升级订单会直接在站内完成，不弹第三方支付窗口

### 套餐升级配置规则

- `upgrade_family` 用于标识“哪些套餐属于同一升级族”，留空表示该套餐不参与升级链
- `upgrade_rank` 用于标识升级梯度，只允许从低等级升到高等级，不能平级升级或降级
- 用户只能升级到当前尚未持有活跃订阅的目标分组，避免重复持有同目标套餐

### 订阅快照与历史兼容

- 系统会在创建或续期订阅时写入当前套餐快照，包括套餐 ID、套餐名、价格、有效期和当前计费周期起点
- 如果管理员后续仅修改套餐名称，系统会同步把引用该 `plan_id` 的订阅展示名更新为最新名称；价格、有效期和计费周期起点等数值快照仍保持原购买/续期时的历史值
- 历史遗留的无快照订阅暂不支持补差价升级，前端会明确提示“暂不支持升级”，需要管理员人工迁移或手工处理
