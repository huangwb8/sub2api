...
func (r *affiliateRepository) EnsureUserAffiliate(ctx context.Context, userID int64) (*service.AffiliateSummary, error) {
	...
	return ensureUserAffiliateWithClient(ctx, clientFromContext(ctx, r.client), userID)
}

func (r *affiliateRepository) GetAffiliateByCode(ctx context.Context, code string) (*service.AffiliateSummary, error) {
	return queryAffiliateByCode(ctx, clientFromContext(ctx, r.client), code)
}
...
VALUES ($1, 'accrue', $2, $3, $4, NOW(), NOW())`, inviterID, amount, inviteeUserID, nullableInt64Arg(sourceOrderID))
		}
		if err != nil {
...
SELECT amount FROM cleared`, userID)
		if err != nil {
		...
		affected, err := txClient.User.Update().Where(dbuser.IDEQ(userID)).AddBalance(transferred).Save(txCtx)
		if err != nil {
...
JOIN user_affiliates inviter_aff ON inviter_aff.user_id = ua.inviter_id`
	total, err := scanAffiliateInt64(ctx, client, "SELECT COUNT(*) "+baseJoin+" "+where, args...)
	if err != nil {
		return nil, 0, err
	}
...
func (r *affiliateRepository) GetAffiliateUserOverview(ctx context.Context, userID int64) (*service.AffiliateUserOverview, error) {
	...
	rows, err := clientFromContext(ctx, r.client).QueryContext(ctx, affiliateUserOverviewSQL, userID)
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
...
func buildAffiliateRecordWhere(filter service.AffiliateRecordFilter, timeColumn string, searchColumns []string) (string, []any) {
	...
	if search != "" && len(searchColumns) > 0 {
		...
		for _, column := range searchColumns {
			parts = append(parts, fmt.Sprintf("LOWER(%s) LIKE $%d", column, len(args)))
		}
...
func queryAffiliateByUserID(ctx context.Context, client affiliateQueryExecer, userID int64) (*service.AffiliateSummary, error) {
	return scanAffiliateSummary(ctx, client, `
SELECT user_id, aff_code, aff_code_custom, aff_rebate_rate_percent, inviter_id,
       aff_count, aff_quota::double precision, aff_frozen_quota::double precision,
...
func scanAffiliateSummary(ctx context.Context, client affiliateQueryExecer, query string, args ...any) (*service.AffiliateSummary, error) {
	...
	if err := rows.Scan(&out.UserID, &out.AffCode, &out.AffCodeCustom, &rebateRate, &inviterID,
		&out.AffCount, &out.AffQuota, &out.AffFrozenQuota, &out.AffHistoryQuota,
...
func (r *affiliateRepository) UpdateUserAffCode(ctx context.Context, userID int64, newCode string) error {
	...
	return r.withTx(ctx, func(txCtx context.Context, txClient *dbent.Client) error {
		...
		res, err := txClient.ExecContext(txCtx, `
UPDATE user_affiliates
SET aff_code = $1, aff_code_custom = true, updated_at = NOW()
...
func (r *affiliateRepository) ResetUserAffCode(ctx context.Context, userID int64) (string, error) {
	...
	err := r.withTx(ctx, func(txCtx context.Context, txClient *dbent.Client) error {
		...
		for i := 0; i < affiliateCodeMaxAttempts; i++ {
			...
			res, err := txClient.ExecContext(txCtx, `
UPDATE user_affiliates
SET aff_code = $1, aff_code_custom = false, updated_at = NOW()
...
func (r *affiliateRepository) SetUserRebateRate(ctx context.Context, userID int64, ratePercent *float64) error {
	...
	return r.withTx(ctx, func(txCtx context.Context, txClient *dbent.Client) error {
		...
		res, err := txClient.ExecContext(txCtx, `
UPDATE user_affiliates
SET aff_rebate_rate_percent = $1, updated_at = NOW()
WHERE user_id = $2`, nullableFloatArg(ratePercent), userID)
		if err != nil {
...
UPDATE user_affiliates
SET aff_rebate_rate_percent = $1, updated_at = NOW()
WHERE user_id = ANY($2)`, nullableFloatArg(ratePercent), pq.Array(userIDs))
		if err != nil {
...
WHERE (ua.aff_code_custom = true OR ua.aff_rebate_rate_percent IS NOT NULL)
  AND (u.email ILIKE $1 OR u.username ILIKE $1)`
	...
	client := clientFromContext(ctx, r.client)
	total, err := scanAffiliateInt64(ctx, client, "SELECT COUNT(*)"+baseFrom, likePattern)
	if err != nil {
		return nil, 0, fmt.Errorf("count affiliate admin entries: %w", err)
	}
	rows, err := client.QueryContext(ctx, `
SELECT ua.user_id, COALESCE(u.email, ''), COALESCE(u.username, ''),
       ua.aff_code, ua.aff_code_custom, ua.aff_rebate_rate_percent, ua.aff_count`+baseFrom+`
ORDER BY ua.updated_at DESC
LIMIT $2 OFFSET $3`, likePattern, pageSize, offset)
	if err != nil {
	...
	for rows.Next() {
		var entry service.AffiliateAdminEntry
		var rebate sql.NullFloat64
		if err := rows.Scan(&entry.UserID, &entry.Email, &entry.Username, &entry.AffCode, &entry.AffCodeCustom, &rebate, &entry.AffCount); err != nil {
			return nil, 0, err
		}
		if rebate.Valid {
			v := rebate.Float64
			entry.AffRebateRatePercent = &v
		}
...
func scanAffiliateInt64(ctx context.Context, client affiliateQueryExecer, query string, args ...any) (int64, error) {
	rows, err := client.QueryContext(ctx, query, args...)
	if err != nil {
		return 0, err
	}
