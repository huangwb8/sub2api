...
func TestResponsesToAnthropic_ReadToolPreservesNonEmptyPages(t *testing.T) {
	resp := &ResponsesResponse{
		ID:     "resp_read_pages",
		Model:  "gpt-5.5",
		Status: "completed",
		Output: []ResponsesOutput{
			{
				Type:      "function_call",
				CallID:    "call_read_pages",
				Name:      "Read",
				Arguments: `{"file_path":"/tmp/demo.pdf","pages":"1-2"}`,
			},
		},
	}
	...
	anth := ResponsesToAnthropic(resp, "claude-opus-4-6")
	require.Len(t, anth.Content, 1)
	assert.JSONEq(t, `{"file_path":"/tmp/demo.pdf","pages":"1-2"}`, string(anth.Content[0].Input))
}
