...
const DefaultCSPPolicy = "default-src 'self'; script-src 'self' __CSP_NONCE__ https://challenges.cloudflare.com https://static.cloudflareinsights.com https://js.stripe.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; img-src 'self' data: https:; font-src 'self' data: https://fonts.gstatic.com; connect-src 'self' https:; frame-src 'self' https://challenges.cloudflare.com https://js.stripe.com; frame-ancestors 'none'; base-uri 'self'; form-action 'self'"
...
type GatewayConfig struct {
	...
	// OpenAIOAuthImagesEnabled 控制 OpenAI OAuth Images → Responses image_generation 桥接。
	// nil 表示使用默认开启；显式 false 可一键回滚到仅 API Key Images。
	OpenAIOAuthImagesEnabled *bool `mapstructure:"openai_oauth_images_enabled"`
	// OpenAIOAuthImagesExperimentalEnabled 兼容旧实验开关。
	// 旧配置 true 视为开启；新配置显式 false 优先生效。
...
type OpsCleanupConfig struct {
	...
	// Retention days (0 truncates that cleanup target on every cleanup run).
	//
...
func setDefaults() {
	...
	viper.SetDefault("gateway.openai_oauth_images_enabled", true)
