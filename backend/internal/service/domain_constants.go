...
const (
	...
	AccountTypeChatAPI    = domain.AccountTypeChatAPI    // Chat Completions API 类型账号（直接转发 /v1/chat/completions）
...
const (
	...
	RedeemTypeInvitationTemp    = domain.RedeemTypeInvitationTemp
	RedeemTypeInvitationBalance = domain.RedeemTypeInvitationBalance
)
...
const (
	...
	SettingKeyTermsOfServiceContent       = "terms_of_service_content"      // 服务条款内容（Markdown）
	SettingKeyPrivacyPolicyContent        = "privacy_policy_content"        // 隐私政策内容（Markdown）
	...
	SettingKeyDashboardResidentialIPPrice   = "dashboard_residential_ip_price_usd_per_gb_month" // Dashboard 套餐测算最近一次使用的住宅 IP 单价
	...
	// Proxy failover has a dedicated API but still shares this persisted container for compatibility.
