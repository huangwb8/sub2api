...
func (s *DashboardService) GetProfitabilityTrend(ctx context.Context, startTime, endTime time.Time, granularity string) ([]usagestats.ProfitabilityTrendPoint, error) {
	type profitabilityTrendRepo interface {
		GetProfitabilityTrend(ctx context.Context, startTime, endTime time.Time, granularity string) ([]usagestats.ProfitabilityTrendPoint, error)
	}

	repo, ok := s.usageRepo.(profitabilityTrendRepo)
	if !ok {
		return nil, fmt.Errorf("get profitability trend: repository does not support profitability trend")
	}
	trend, err := repo.GetProfitabilityTrend(ctx, startTime, endTime, granularity)
	if err != nil {
		return nil, fmt.Errorf("get profitability trend: %w", err)
	}
	return trend, nil
}
