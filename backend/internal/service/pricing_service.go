...
var (
	...
	openAIGPT55FallbackPricing = &LiteLLMModelPricing{
		...
		CacheCreationInputTokenCost:     5e-06,    // 首次写缓存按输入价计费
...
func (s *PricingService) matchByModelFamily(model string) *LiteLLMModelPricing {
	...
	if matched == nil {
		if strings.Contains(model, "opus") {
			if strings.Contains(model, "4.7") || strings.Contains(model, "4-7") {
				matched = &modelFamily{name: "opus-4.7", pricing: []string{"claude-opus-4-7", "claude-opus-4.7", "claude-opus-4-6"}}
			} else if strings.Contains(model, "4.5") || strings.Contains(model, "4-5") {
				matched = &modelFamily{name: "opus-4.5", pricing: []string{"claude-opus-4-5", "claude-opus-4.5"}}
			} else {
				matched = &modelFamily{name: "opus-4", pricing: []string{"claude-opus-4", "claude-3-opus"}}
			}
		} else if strings.Contains(model, "sonnet") {
			if strings.Contains(model, "4.5") || strings.Contains(model, "4-5") {
				matched = &modelFamily{name: "sonnet-4.5", pricing: []string{"claude-sonnet-4-5", "claude-sonnet-4.5"}}
			} else if strings.Contains(model, "3-5") || strings.Contains(model, "3.5") {
				matched = &modelFamily{name: "sonnet-3.5", pricing: []string{"claude-3-5-sonnet", "claude-3.5-sonnet"}}
			} else {
				matched = &modelFamily{name: "sonnet-4", pricing: []string{"claude-sonnet-4", "claude-3-5-sonnet"}}
			}
		} else if strings.Contains(model, "haiku") {
			if strings.Contains(model, "3-5") || strings.Contains(model, "3.5") {
				matched = &modelFamily{name: "haiku-3.5", pricing: []string{"claude-3-5-haiku", "claude-3.5-haiku"}}
			} else {
				matched = &modelFamily{name: "haiku-3", pricing: []string{"claude-3-haiku"}}
			}
		}
	}
	...
	for _, pattern := range matched.pricing {
		for key, pricing := range s.pricingData {
