...
func TestNormalizeOpenAICompatRequestedModel(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "gpt55 reasoning alias strips high", input: "gpt-5.5-high", want: "gpt-5.5"},
...
func TestApplyOpenAICompatModelNormalization(t *testing.T) {
	...
	t.Run("derives high from gpt55 suffix when output config missing", func(t *testing.T) {
		req := &apicompat.AnthropicRequest{Model: "gpt-5.5-high"}
...
func TestForwardAsAnthropic_APIKeyInjectsPromptCacheKeyIntoResponsesBody(t *testing.T) {
	t.Parallel()
	gin.SetMode(gin.TestMode)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_apikey_prompt_cache"}},
		Body:       io.NopCloser(strings.NewReader(upstreamBody)),
	}}
	...
	result, err := svc.ForwardAsAnthropic(context.Background(), c, account, body, "pc-123", "gpt-5.1")
	require.NoError(t, err)
	require.NotNil(t, result)
	require.Equal(t, "pc-123", gjson.GetBytes(upstream.lastBody, "prompt_cache_key").String())
}

func TestForwardAsAnthropic_BufferedResponseDoneSupplementsDeltaOutput(t *testing.T) {
	t.Parallel()
	gin.SetMode(gin.TestMode)
	...
	upstreamBody := strings.Join([]string{
		...
		"",
		`data: {"type":"response.done","response":{"id":"resp_1","object":"response","model":"gpt-5.4","status":"completed","output":[],"usage":{"input_tokens":5,"output_tokens":2,"total_tokens":7}}}`,
		"",
		"data: [DONE]",
		"",
	}, "\n")
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_buffered_done"}},
		Body:       io.NopCloser(strings.NewReader(upstreamBody)),
	}}
