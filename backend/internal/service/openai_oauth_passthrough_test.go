...
func TestOpenAIGatewayService_OAuthPassthrough_CompactUsesJSONAndKeepsNonStreaming(t *testing.T) {
	...
	originalBody := []byte(`{"model":"gpt-5.1-codex","stream":true,"store":true,"instructions":"local-test-instructions","tools":[{"type":"function","name":"bash"}],"parallel_tool_calls":false,"reasoning":{"effort":"high"},"text":{"format":{"type":"text"}},"temperature":1,"top_p":0.7,"input":[{"type":"text","text":"compact me"}]}`)
	...
	require.Equal(t, "text", gjson.GetBytes(upstream.lastBody, "text.format.type").String())
...
func TestOpenAIGatewayService_ChatAPIPassthrough_PreservesBodyAndUsesResponsesEndpoint(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	account := &Account{
		...
		Concurrency:    1,
		Credentials:    map[string]any{"api_key": "sk-chatapi-key", "base_url": "https://api.openai.com"},
		Extra:          map[string]any{"openai_passthrough": true},
		Status:         StatusActive,
		Schedulable:    true,
		RateMultiplier: f64p(1),
	...
	require.Equal(t, originalBody, upstream.lastBody)
	require.Equal(t, "https://api.openai.com/v1/responses", upstream.lastReq.URL.String())
	require.Equal(t, "Bearer sk-chatapi-key", upstream.lastReq.Header.Get("Authorization"))
	require.Equal(t, "curl/8.0", upstream.lastReq.Header.Get("User-Agent"))
	require.Empty(t, upstream.lastReq.Header.Get("X-Test"))
}

func TestOpenAIGatewayService_ChatAPINonPassthrough_RemovesResponsesOnlyTokenFields(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	account := &Account{
		ID:          458,
		Name:        "chatapi-standard",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeChatAPI,
		Concurrency: 1,
		Credentials: map[string]any{
			"api_key":  "sk-chatapi-key",
	...
	_, err := svc.Forward(context.Background(), c, account, []byte(`{"model":"gpt-5.2","stream":false,"max_output_tokens":128,"max_completion_tokens":64,"input":[{"type":"text","text":"hi"}]}`))
	require.NoError(t, err)
	require.NotNil(t, upstream.lastReq)
...
func TestOpenAIGatewayService_ChatCompletionsBufferedMissingTerminalTriggersFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	resp := &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "X-Request-Id": []string{"rid-missing-terminal"}},
		Body:       io.NopCloser(strings.NewReader("data: {\"type\":\"response.output_text.delta\",\"delta\":\"h\"}\n\n")),
	}
	upstream := &httpUpstreamRecorder{resp: resp}
	svc := &OpenAIGatewayService{
	...
	require.Contains(t, string(failoverErr.ResponseBody), "terminal response event")
	require.Empty(t, rec.Body.String())
}

func TestOpenAIGatewayService_MessagesBufferedMissingTerminalTriggersFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)

	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	body := []byte(`{"model":"claude-3-5-sonnet-20241022","messages":[{"role":"user","content":"hi"}],"max_tokens":1024,"stream":false}`)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/messages", bytes.NewReader(body))
	c.Request.Header.Set("Content-Type", "application/json")

	resp := &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "X-Request-Id": []string{"rid-messages-missing-terminal"}},
		Body:       io.NopCloser(strings.NewReader("data: {\"type\":\"response.output_text.delta\",\"delta\":\"h\"}\n\n")),
	}
	upstream := &httpUpstreamRecorder{resp: resp}
	svc := &OpenAIGatewayService{
