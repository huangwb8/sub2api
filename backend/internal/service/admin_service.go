...
// AdminResetAPIKeyRateLimitUsage resets all rate-limit usage windows for an API key.
...
func (s *adminServiceImpl) resolveBulkUpdateTargetIDs(ctx context.Context, filters *BulkUpdateAccountFilters) ([]int64, error) {
	...
	groupID := int64(0)
	switch group := strings.TrimSpace(filters.Group); group {
	case "":
	case "ungrouped":
		groupID = AccountListGroupUngrouped
	default:
		parsedGroupID, err := strconv.ParseInt(group, 10, 64)
		if err != nil {
