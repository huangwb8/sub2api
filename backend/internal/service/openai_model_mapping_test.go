...
func TestResolveOpenAIForwardModel_PreservesUnknownModelWithoutDefault(t *testing.T) {
	account := &Account{
		Credentials: map[string]any{},
	}
...
func TestNormalizeOpenAIModelForUpstream(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "oauth preserves unknown compatible model",
		...
		{
			name:    "oauth preserves gpt55 reasoning family",
			account: &Account{Type: AccountTypeOAuth},
			model:   "gpt-5.5-high",
			want:    "gpt-5.5",
		},
