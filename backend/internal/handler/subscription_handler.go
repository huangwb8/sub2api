...
type SubscriptionHandler struct {
	...
	subscriptionUpgradeService *service.SubscriptionUpgradeService
}

// NewSubscriptionHandler creates a new user subscription handler
func NewSubscriptionHandler(subscriptionService *service.SubscriptionService, subscriptionUpgradeService *service.SubscriptionUpgradeService) *SubscriptionHandler {
...
// GetUpgradeOptions handles listing upgrade options for a specific active subscription.
// GET /api/v1/subscriptions/:id/upgrade-options
func (h *SubscriptionHandler) GetUpgradeOptions(c *gin.Context) {
	subject, ok := middleware2.GetAuthSubjectFromContext(c)
	if !ok {
		response.Unauthorized(c, "User not found in context")
		return
	...
	result, err := h.subscriptionUpgradeService.ListUpgradeOptions(c.Request.Context(), subject.UserID, subscriptionID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
