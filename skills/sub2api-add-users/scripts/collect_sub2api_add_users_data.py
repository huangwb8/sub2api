#!/usr/bin/env python3
"""Read-only collector for sub2api add-user capacity analysis."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Collect read-only sub2api add-user capacity data.")
    ...
    parser.add_argument(
        "--work-root",
        default=os.getenv("SUB2API_ADD_USERS_WORK_ROOT", str(config_directories["work_root"])),
    )
...
def parse_time(value: str, fallback: dt.datetime, default_tz: dt.tzinfo) -> dt.datetime:
    if not value:
        return fallback
...
def collect() -> int:
    ...
    user_tz = timezone_from_name(args.timezone)
    until = parse_time(args.until, now, user_tz)
    since = parse_time(args.since, until - dt.timedelta(days=int_default(config_defaults, "lookback_days")), user_tz)
    if since >= until:
    ...
    endpoints: list[tuple[str, str, dict[str, Any], bool]] = [
        ("recommendations.json", "/admin/dashboard/recommendations", {}, True),
        ("usage_stats.json", "/admin/usage/stats", common, True),
        ("capacity_summary.json", "/admin/groups/capacity-summary", {}, False),
        (
    ...
    write_json(out_dir / "_metadata.json", metadata)
    write_json(out_dir / "_errors.json", errors)
    print(f"Collected sub2api add-user data into {out_dir}")
    return 0
