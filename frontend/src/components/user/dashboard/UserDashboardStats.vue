    ...
    <div v-if="!isSimple" class="card p-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xl font-bold text-emerald-600 dark:text-emerald-400">{{ formatBalanceAmount(balance) }}</p>
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xl font-bold text-gray-900 dark:text-white">
            <span class="text-purple-600 dark:text-purple-400" :title="t('dashboard.actual')">{{ formatUsageCost(stats?.today_actual_cost || 0, { minimumFractionDigits: 4, maximumFractionDigits: 4 }) }}</span>
            <span class="text-sm font-normal text-gray-400 dark:text-gray-500" :title="t('dashboard.standard')"> / {{ formatUsageCost(stats?.today_cost || 0, { minimumFractionDigits: 4, maximumFractionDigits: 4 }) }}</span>
          </p>
          <p class="text-xs">
            <span class="text-gray-500 dark:text-gray-400">{{ t('common.total') }}: </span>
            <span class="text-purple-600 dark:text-purple-400" :title="t('dashboard.actual')">{{ formatUsageCost(stats?.total_actual_cost || 0, { minimumFractionDigits: 4, maximumFractionDigits: 4 }) }}</span>
            <span class="text-gray-400 dark:text-gray-500" :title="t('dashboard.standard')"> / {{ formatUsageCost(stats?.total_cost || 0, { minimumFractionDigits: 4, maximumFractionDigits: 4 }) }}</span>
          </p>
        </div>
      </div>
    </div>
...
import { formatBalanceAmount, formatUsageCost } from '@/utils/format'
