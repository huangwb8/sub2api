...
export const METHOD_ORDER = ['balance', 'alipay', 'alipay_direct', 'wxpay', 'wxpay_direct', 'stripe'] as const
 ...
 * needs a wider window to avoid clipped QR/login panels. */
const PAYMENT_POPUP_PREFERRED_WIDTH = 1250
const PAYMENT_POPUP_PREFERRED_HEIGHT = 900

/** Build a centered popup window that fits the available work area. */
export function getPaymentPopupFeatures(): string {
  ...
  const fitW = availW > 40 ? availW - 40 : availW
  const fitH = availH > 40 ? availH - 40 : availH
  const width = Math.min(PAYMENT_POPUP_PREFERRED_WIDTH, Math.max(0, fitW))
  const height = Math.min(PAYMENT_POPUP_PREFERRED_HEIGHT, Math.max(0, fitH))
