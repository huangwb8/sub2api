    ...
    <div v-if="order" class="space-y-4">
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ formatPaymentAmount(order.amount) }}</p>
        </div>
        <div>
          <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.orders.payAmount') }}</p>
          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ formatPaymentAmount(order.pay_amount) }}</p>
        </div>
        <div>
      ...
      >
        ...
        <div class="grid grid-cols-2 gap-2 text-sm">
          <div>
            ...
            <span class="ml-1 font-medium text-red-700 dark:text-red-300">{{ formatPaymentAmount(order.refund_amount) }}</span>
          </div>
