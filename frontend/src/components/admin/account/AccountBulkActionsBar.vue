<template>
  <div v-if="selectedIds.length > 0" class="mb-4 flex items-center justify-between p-3 bg-primary-50 rounded-lg dark:bg-primary-900/20">
    ...
    <div class="flex gap-2">
      ...
      <button @click="$emit('edit-selected')" class="btn btn-primary btn-sm">{{ t('admin.accounts.bulkActions.editSelected') }}</button>
      <button @click="$emit('edit-filtered')" class="btn btn-secondary btn-sm">{{ t('admin.accounts.bulkActions.editFiltered') }}</button>
    </div>
  </div>
</template>
