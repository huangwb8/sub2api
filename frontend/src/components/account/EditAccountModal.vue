<template>
  ...
  >
    ...
    >
      ...
      <div v-if="account.type === 'apikey' || account.type === 'chatapi'" class="space-y-4">
        <div>
      ...
      <div
        v-if="account?.platform === 'openai' && account?.type === 'chatapi'"
        class="border-t border-gray-200 pt-4 dark:border-dark-600"
      >
        <div class="flex items-center justify-between">
          <div>
            <label class="input-label mb-0">{{ t('admin.accounts.openai.chatAPIResponsesEnabled') }}</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.openai.chatAPIResponsesEnabledDesc') }}
            </p>
          </div>
          <button
            type="button"
            @click="chatAPIResponsesEnabled = !chatAPIResponsesEnabled"
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2',
              chatAPIResponsesEnabled ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
            ]"
          >
            <span
              :class="[
                'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                chatAPIResponsesEnabled ? 'translate-x-5' : 'translate-x-0'
              ]"
            />
          </button>
        </div>
      ...
      <div v-if="account?.type === 'apikey' || account?.type === 'chatapi' || account?.type === 'bedrock'" class="border-t border-gray-200 pt-4 dark:border-dark-600 space-y-4">
        <div class="mb-3">
...
const chatAPIResponsesEnabled = ref(false)
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  chatAPIResponsesEnabled.value = false
  ...
  if (newAccount.platform === 'openai' && newAccount.type === 'chatapi') {
    chatAPIResponsesEnabled.value = extra?.chatapi_responses_enabled === true
  }
  ...
  if (newAccount.type === 'apikey' || newAccount.type === 'chatapi' || newAccount.type === 'bedrock') {
  ...
  if ((newAccount.type === 'apikey' || newAccount.type === 'chatapi') && newAccount.credentials) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.account.type === 'apikey' || props.account.type === 'chatapi') {
    ...
    if (props.account.platform === 'openai' && props.account.type === 'chatapi') {
      const currentExtra = (props.account.extra as Record<string, unknown>) || {}
      const newExtra: Record<string, unknown> = { ...currentExtra }
      if (chatAPIResponsesEnabled.value) {
        newExtra.chatapi_responses_enabled = true
      } else {
        delete newExtra.chatapi_responses_enabled
      }
      updatePayload.extra = newExtra
    }

    // For apikey/bedrock accounts, handle quota_limit in extra
    if (props.account.type === 'apikey' || props.account.type === 'chatapi' || props.account.type === 'bedrock') {
