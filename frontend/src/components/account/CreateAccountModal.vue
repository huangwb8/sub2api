    ...
    >
      ...
      <div v-if="form.platform === 'openai'">
        ...
        <div class="mt-2 grid grid-cols-3 gap-3" data-tour="account-form-type">
          ...
          <button
            type="button"
            @click="accountCategory = 'chatapi'"
            :class="[
              'flex items-center gap-3 rounded-lg border-2 p-3 text-left transition-all',
              accountCategory === 'chatapi'
          ...
          >
            ...
            <div>
              <span class="block text-sm font-medium text-gray-900 dark:text-white">
                {{ t('admin.accounts.types.chatCompletionsApi') }}
              </span>
              <span class="text-xs text-gray-500 dark:text-gray-400">
                {{ t('admin.accounts.types.chatCompletionsDesc') }}
              </span>
            </div>
          </button>
        </div>
      ...
      <!-- API Key-like input (excluding Antigravity which has its own fields) -->
      <div v-if="(form.type === 'apikey' || form.type === 'chatapi') && form.platform !== 'antigravity'" class="space-y-4">
        <div>
      ...
      <!-- API Key / ChatAPI / Bedrock 账号配额限制 -->
      <div v-if="form.type === 'apikey' || form.type === 'chatapi' || form.type === 'bedrock'" class="border-t border-gray-200 pt-4 dark:border-dark-600 space-y-4">
        <div class="mb-3">
      ...
      <div
        v-if="form.platform === 'openai' && accountCategory !== 'chatapi'"
        class="border-t border-gray-200 pt-4 dark:border-dark-600"
      >
        <div class="flex items-center justify-between">
          <div>
...
const accountCategory = ref<'oauth-based' | 'apikey' | 'chatapi' | 'bedrock'>('oauth-based') // UI selection for account category
...
const form = reactive({
  ...
  concurrency: 5 as NumericInputValue,
...
const effectiveCreateAccountType = computed<AccountType>(() => {
  if (form.platform === 'antigravity' && antigravityAccountType.value === 'upstream') {
    return 'apikey'
  }
  if (form.platform === 'anthropic' && accountCategory.value === 'bedrock') {
  ...
  if (form.platform === 'openai' && accountCategory.value === 'chatapi') {
    return 'chatapi'
  }
...
const mechanismMatchesCurrentAccount = (mechanism: SchedulingMechanism) => {
  if (!mechanism.enabled || mechanism.temp_unschedulable_enabled === false) return false
  const platform = mechanism.platform || 'all'
  const accountType = mechanism.account_type || 'all'
  return (platform === 'all' || platform === form.platform) &&
    (accountType === 'all' || accountType === effectiveCreateAccountType.value)
}
...
watch(
  ...
  ([category, method, agType]) => {
    ...
    if (form.platform === 'openai' && category === 'chatapi') {
      form.type = 'chatapi' as AccountType
      return
    }
...
const loadSchedulingMechanisms = async () => {
...
const buildOpenAIExtra = (base?: Record<string, unknown>): Record<string, unknown> | undefined => {
  ...
  if (accountCategory.value === 'chatapi') {
    return base
  }
...
const createAccountAndFinish = async (
  ...
  if (type === 'apikey' || type === 'chatapi' || type === 'bedrock') {
...
const handleCookieAuth = async (sessionKey: string) => {
  ...
  try {
    ...
    const tempUnschedPayload = tempUnschedEnabled.value
      ...
      : []
    const tempUnschedRuleRefsPayload = tempUnschedEnabled.value
      ? buildTempUnschedRuleRefs()
      : []
    if (tempUnschedEnabled.value && tempUnschedPayload.length === 0 && tempUnschedRuleRefsPayload.length === 0) {
      appStore.showError(t('admin.accounts.tempUnschedulable.rulesInvalid'))
      return
    }
    ...
    for (let i = 0; i < keys.length; i++) {
      try {
        ...
        if (tempUnschedEnabled.value) {
          credentials.temp_unschedulable_enabled = true
          if (tempUnschedPayload.length > 0) {
            credentials.temp_unschedulable_rules = tempUnschedPayload
          }
          if (tempUnschedRuleRefsPayload.length > 0) {
            credentials.temp_unschedulable_rule_refs = tempUnschedRuleRefsPayload
          }
        }
