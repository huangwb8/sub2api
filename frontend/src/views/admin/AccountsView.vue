    ...
    <TablePageLayout>
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-actual_cost_cny="{ row }">
            <span
              v-if="row.actual_cost_cny != null && row.actual_cost_cny > 0"
              class="text-sm font-mono text-gray-700 dark:text-gray-300"
            >
              ¥{{ row.actual_cost_cny.toFixed(2) }}
            </span>
            <span v-else class="text-sm text-gray-400 dark:text-dark-500">-</span>
          </template>
...
const allColumns = computed(() => {
  ...
  c.push(
    ...
    { key: 'actual_cost_cny', label: t('admin.accounts.columns.actualCostCny'), sortable: true },
...
const openBulkEditFiltered = async () => {
  try {
    ...
    console.error('Failed to preview filtered accounts:', error)
    appStore.showError(t('admin.accounts.failedToLoad'))
  }
}

const handleBulkEditClosed = () => {
  showBulkEdit.value = false
  bulkEditTarget.value = null
}

const handleBulkUpdated = () => {
  handleBulkEditClosed()
  clearSelection()
  reload()
}
...
const accountMatchesCurrentFilters = (account: Account) => {
  ...
  if (!matchesSearchTerms(String(filters.search || ''), [account.name])) return false
  return true
}
