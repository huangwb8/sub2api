  ...
  <AppLayout>
    <div class="mx-auto max-w-4xl space-y-6">
      <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
        <StatCard :title="t('profile.accountBalance')" :value="formatBalanceAmount(user?.balance || 0)" :icon="WalletIcon" icon-variant="success" />
      ...
      <div class="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800 dark:border-emerald-900/60 dark:bg-emerald-950/30 dark:text-emerald-200">
        余额与订单按 CNY 记账，按量调用成本仍按 USD 计算，并在结算时按汇率快照折算为 CNY 扣减。
      </div>
...
import { useAuthStore } from '@/stores/auth'; import { formatBalanceAmount, formatDate } from '@/utils/format'
